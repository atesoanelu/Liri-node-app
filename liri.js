...
function doit(action,data) {
    ...
    switch(action) {
        ...
        case 'spotify-this-song':
            ...
            spotify.search({ type: 'track', query: data, limit: 1 }, function(err, response) {
                ...
                if (data) {
                    ...
                    console.log('Spotify URL: '+response.tracks.items[0].artists[0].external_urls.spotify);
                    console.log('Album Name: '+response.tracks.items[0].album.name);
                    append('Artist Name: '+response.tracks.items[0].artists[0].name+'\n');
                    append('Song Name: '+response.tracks.items[0].name+'\n');
                    append('Spotify URL: '+response.tracks.items[0].artists[0].external_urls.spotify+'\n');
                    append('Album Name: '+response.tracks.items[0].album.name+'\n');
        ...
        case 'movie-this':
            ...
            request('http://www.omdbapi.com/?t='+data+'&y=&plot=short&tomatoes=true&r=json&apikey=40e9cece', function (error, response, body) {
